"""Сверка конвейерного compute_core_pipe с Python."""
...
def gen_input(ncase=10):
    ...
    with open(os.path.join(SIM, "ccp_in.hex"), "w") as f:
        with open(os.path.join(SIM, "ccp_expected.hex"), "w") as fe:
...
def run_sim():
    files = [
        ...
        os.path.join(RTL, "rtl", "compute_core_pipe.sv"),
        os.path.join(RTL, "tb", "tb_compute_pipe.sv"),
    ]
    ...
    run(f'"{xelab}" tb_compute_pipe -debug typical')
    run(f'"{xsim}" tb_compute_pipe -runall')


def verify(tol=2):
    exp = [int(l.strip(), 16) for l in open(os.path.join(SIM, "ccp_expected.hex")) if l.strip()]
    got = [int(l.strip(), 16) for l in open(os.path.join(SIM, "ccp_out.hex")) if l.strip()]
    if len(exp) != len(got):
    ...
    if bad == 0:
        print(f"PIPE(N={N}): все {len(exp)} совпали (PASS, tol={tol})")
        return True
    print(f"PIPE: несовпадений {bad}/{len(exp)}")
    return False
