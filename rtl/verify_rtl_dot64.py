"""Сверка RTL-векторного ядра dot N=64 с Python-эталоном."""
...
def gen_input(ncase=10):
    ...
    cases = []
    with open(os.path.join(SIM, "dot64_in.hex"), "w") as f:
        with open(os.path.join(SIM, "dot64_expected.hex"), "w") as fe:
            for c in range(ncase):
                a = [f32(random.uniform(-10, 10)) for _ in range(N)]
                b = [f32(random.uniform(-10, 10)) for _ in range(N)]
                ta = [TFloat.from_float(x) for x in a]
                tb = [TFloat.from_float(x) for x in b]
                f.write(" ".join(f"{t.to_bits():010x}" for t in ta + tb) + "\n")
                r = dot_ref(a, b)
                fe.write(f"{r.to_bits():010x}\n")
                cases.append((a, b))
    print(f"Сгенерировано {len(cases)} случаев N={N}")
    return cases
...
def run_sim():
    files = [
        ...
        os.path.join(RTL, "tb", "tb_tf40_dot64.sv"),
    ]
    ...
    run(f'"{xelab}" tb_tf40_dot64 -debug typical')
    run(f'"{xsim}" tb_tf40_dot64 -runall')


def verify():
    exp = [int(l.strip(), 16) for l in open(os.path.join(SIM, "dot64_expected.hex")) if l.strip()]
    got = [int(l.strip(), 16) for l in open(os.path.join(SIM, "dot64_out.hex")) if l.strip()]
    if len(exp) != len(got):
