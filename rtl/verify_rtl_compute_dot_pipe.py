"""Сверка конвейерного compute_core_dot_pipe (N=16) с Python-эталоном."""
...
def gen_input(ncase=10):
    ...
    with open(os.path.join(SIM, "ccdp_in.hex"), "w") as f:
        with open(os.path.join(SIM, "ccdp_expected.hex"), "w") as fe:
...
def run_sim():
    files = [
        ...
        os.path.join(RTL, "rtl", "compute_core_dot_pipe.sv"),
        os.path.join(RTL, "tb", "tb_compute_dot_pipe.sv"),
    ]
    ...
    run(f'"{xelab}" tb_compute_dot_pipe -debug typical')
    run(f'"{xsim}" tb_compute_dot_pipe -runall')


def verify(tol=2):
    exp = [int(l.strip(), 16) for l in open(os.path.join(SIM, "ccdp_expected.hex")) if l.strip()]
    got = [int(l.strip(), 16) for l in open(os.path.join(SIM, "ccdp_out.hex")) if l.strip()]
    if len(exp) != len(got):
    ...
    if bad == 0:
        print(f"COMPUTE_DOT_PIPE(N={N}): все {len(exp)} совпали (PASS, tol={tol})")
        return True
    print(f"COMPUTE_DOT_PIPE: несовпадений {bad}/{len(exp)}")
    return False
