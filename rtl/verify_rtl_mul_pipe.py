"""Сверка конвейерного tf40_mul_pipe с Python-эталоном."""
...
def gen_input(n=50):
    ...
    with open(os.path.join(SIM, "mulp_in.hex"), "w") as f:
        for x, y in pairs:
            ta, tb = TFloat.from_float(x), TFloat.from_float(y)
            f.write(f"{ta.to_bits():010x} {tb.to_bits():010x}\n")
    with open(os.path.join(SIM, "mulp_expected.hex"), "w") as f:
        for x, y in pairs:
            ta, tb = TFloat.from_float(x), TFloat.from_float(y)
            r = arith.mul(ta, tb)
...
def run_sim():
    files = [
        ...
        os.path.join(RTL, "tb", "tb_tf40_mul_pipe.sv"),
    ]
    ...
    run(f'"{xelab}" tb_tf40_mul_pipe -debug typical')
    run(f'"{xsim}" tb_tf40_mul_pipe -runall')


def verify():
    exp = [int(l.strip(), 16) for l in open(os.path.join(SIM, "mulp_expected.hex")) if l.strip()]
    got = [int(l.strip(), 16) for l in open(os.path.join(SIM, "mulp_out.hex")) if l.strip()]
    # первые (латентность) пропускаем, сравниваем по количеству
    ...
    print(f"MUL_PIPE: сравнено {n} (expected={len(exp)}, got={len(got)}), несовпадений {bad}")
    return bad == 0
