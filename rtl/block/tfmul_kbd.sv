...
//
// СТАТУС/АНАЛИЗ ЦЕЛЕСООБРАЗНОСТИ (Task 17, 2026-09-06): исходные обещания
// «экономия ~69% LUT (55k→17k)» и «8 тактов vs 25» ОШИБОЧНЫ и удалены.
// Количественная проверка (подготовка к пайплайнингу):
//   * параллельный mul_10x10 = 20 колонок × 10-входовые троичные суммы
//     (~35-50 LUT/колонку) ≈ ~1000 LUT/умножитель; 3-4 таких на lane =
//     3-4k LUT против ~0.9-1.5k у ПОСЛЕДОВАТЕЛЬНОГО tfmul_raw (1 общий
//     tbyte_mul, 40 колонок × 3-входовые суммы). Parallel-вариант в 2-3 раза
//     ДОРОЖЕ по LUT на всей иерархии (×NUM_MAC);
//   * групповой пайплайн серийных balanced-цепочек без carry-lookahead даёт
//     ~35-50 стадий — латентностью НЕ лучше raw (~49 тактов после 7da39e5);
//   * латентность dot-операции доминирует дерево tfadd_raw (для NUM_MAC=32:
//     31 ПОСЛЕДОВАТЕЛЬНОЕ сложение × ~30-50 тактов, ALGN/NORM циклические),
//     т.е. ускорение умножителя даёт ~2-3% на точку.
// ВЫВОД: в сборке осознанно остаётся tfmul_raw (тайминг-clean, дёшев);
// tfmul_kbd — точный эталон (BUG-039) для тестов/модели и резерв для
// будущих DSP-/carry-lookahead-вариантов, если PH_MUL станет узким местом.
// Переключение возможно только после пайплайна + MUL_IMPL + замера стенда.
// ============================================================================
module tfmul_kbd (
    input  logic        clk,
    input  logic        rst_n,
