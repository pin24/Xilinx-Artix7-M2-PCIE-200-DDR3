// ============================================================================
// ternary_booth_recoder.sv — троичный Booth recoding множителя (OPTIONAL)
// ============================================================================
// ВАЖНО: Для balanced ternary {-1, 0, +1} классический Booth recoding
// НЕ даёт выигрыша в числе partial products, потому что:
//   - Каждый трит уже ∈ {-1, 0, +1} (sparse ~32% нулей в случайных числах)
//   - Группировка 2 тритов даёт значения [-4, +4], но ±2A, ±3A в троичной
//     арифметике — это сложения (нельзя сдвигом, как в двоичной Booth)
//   - Дополнительные LUT на генерацию ±2A, ±3A съедают потенциальную экономию
//
// Этот модуль оставлен как REFERENCE для будущих экспериментов (например,
// для не-balanced ternary или если найдете вариант Booth, который работает
// для троичной системы). В tfmul_kbd.sv он НЕ используется — там применяется
// sign-mux (трит × вектор = ±вектор), что эквивалентно "1 partial product
// на трит" с Dadda tree для сжатия.
//
// Конвертирует 20-тритный множитель в 11 "recoded" групп, каждая ∈
