...
// ============================================================================
// GOLDEN SERIAL-КОПИЯ (переименована в tfadd_raw_ref) — заморожена как
// эталон для tb_tfadd_equiv.sv (A/B-эквивалентность barrel-версии).
// ВНИМАНИЕ: содержит BUG-040 (cnt[5:0], обрезка |Δe| mod 64) — при
// |Δe| >= 64 barrel-версия ПРАВА, а эта копия расходится (см. шапку
// tfadd_raw.sv). Референс валиден при |Δe| <= 63.
// ВНИМАНИЕ 2 (BUG-041, 2026-09-06): содержит ту же нулевую дыру, что и serial
// ДО фикса — нулевой операнд с «большой» экспонентой съедает ненулевой
// (big выбирается по e, нулевой prod несёт мусорную e). Barrel-версия ПРАВА
// и здесь (x+0 == norm(x)); tb_tfadd_equiv учитывает зону отдельно (Z41).
// НЕ подключается к сборке (build_dfx.tcl / synth_par_raw.tcl — явные
// списки файлов без tfadd_raw_ref.sv).
// ============================================================================
module tfadd_raw_ref (
    input  logic        clk,
    input  logic        rst_n,
