"""Сверка параллельного compute_dot_par_raw (NUM_MAC, TFloat48) с Python-моделью.

Эталон повторяет алгоритм RTL compute_dot_par_raw:
  - продукты НЕ нормализуются (как tfmul_raw), нормализация только на дереве
    сложений (как tfadd_raw): выравнивание делением /3^cnt с round-half-up,
    поразрядное сложение мантисс, нормализация к [3^18, 3^19).
"""
...
# ---- тритовые примитивы (точная копия RTL rhu_next / norm) ----
...
def _rhu_next(m, W=42):
    """/3 с round-half-up (точная копия RTL rhu_next)."""
    ts = _to_trits(m, W)
    sign_neg = 0
...
def _norm_raw_rtl(m, e):
    """нормализация к [3^18,3^19) (точная копия RTL PH_NORM со знаком)."""
    W = 42
    while True:
        ts = _to_trits(m, W)
...
def run_sim():
    ...
    tb_path = os.path.join(RTL, "tb_compute_dot_par_raw.sv")
    ...
    files = [
        ...
        os.path.join(RTL, "tfadd_raw.sv"),
        os.path.join(RTL, "tfmul_raw.sv"),
        os.path.join(RTL, "compute_dot_par_raw.sv"),
        tb_use,
    ]
    ...
    run(f'"{xelab}" tb_compute_dot_par_raw -debug typical')
    run(f'"{xsim}" tb_compute_dot_par_raw -runall')


def verify():
    exp = [int(l.strip(), 16) for l in open(os.path.join(SIM, "cdparr_expected.hex")) if l.strip()]
    got = [int(l.strip(), 16) for l in open(os.path.join(SIM, "cdparr_out.hex")) if l.strip()]
    if len(exp) != len(got):
