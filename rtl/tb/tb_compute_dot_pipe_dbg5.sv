// ============================================================================
// tb_compute_dot_pipe_dbg5.sv - трассировка подачи f2t_valid_in по тактам
// ============================================================================
module tb_compute_dot_pipe_dbg5;
    localparam int N = 16;
    logic clk = 0, rst_n = 0;
    ...
    initial begin
        ...
        for (int t = 0; t < 300; t++) begin
            @(posedge clk);
            if (dut.phase == 1)
                $display("T%0d cv=%0d vin=%b din=%h", t, dut.cv_i, dut.f2t_valid_in, dut.f2t_data_in);
            if (dut.phase == 2)
                break;
        end
        $finish;
