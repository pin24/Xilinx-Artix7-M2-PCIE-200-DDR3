// ============================================================================
// tb_compute_dot_pipe_dbg6.sv - все пары приёма F2T в compute_core
// ============================================================================
module tb_compute_dot_pipe_dbg6;
    localparam int N = 16;
    logic clk = 0, rst_n = 0;
    ...
    initial begin
        ...
        for (int t = 0; t < 300; t++) begin
            ...
            if (dut.f2t_valid_out)
                $display("RCV[%0d]: %h (din был %h)", dut.cr_i, dut.f2t_data_out, dut.f2t_data_in);
            if (dut.phase == 2)
                break;
        end
        $finish;
