// ============================================================================
// tb_f2t_pipe2_fsm2.sv - F2T FSM-подача + печать захвата входа
// ============================================================================
module tb_f2t_pipe2_fsm2;
    logic clk=0, rst_n=0;
    logic s_valid;
    logic [31:0] s_data;
    logic [39:0] m_data;
    ...
    initial begin
        ...
        for (int t = 0; t < 300; t++) begin
            @(posedge clk);
            // печать захвата входа на последних тактах подачи
            if (s_valid)
                $display("CAP cv=%0d din=%h", cv_i, s_data);
            if (m_valid) begin
                if (cnt >= 27)
