// ============================================================================
// tb_compute_dot_pipe_dbg3.sv - печать a_tf/b_tf после конвертации
// ============================================================================
module tb_compute_dot_pipe_dbg3;
    localparam int N = 16;
    logic clk = 0, rst_n = 0;
    ...
    initial begin
        ...
        for (int t = 0; t < 600; t++) begin
            @(posedge clk);
            if (dut.phase == 2 && dut.pr_i == 0) begin
                $display("=== a_tf / b_tf ===");
                for (int i = 0; i < N; i++)
                    $display("  [%0d] a=%h b=%h", i, dut.a_tf[i], dut.b_tf[i]);
                break;
            end
        end
        $finish;
