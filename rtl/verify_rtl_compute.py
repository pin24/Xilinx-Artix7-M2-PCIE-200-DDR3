"""Сквозная сверка compute_core (f32 -> dot -> f32) с Python.

Запуск:  python verify_rtl_compute.py
"""
...
def gen_input(ncase=20):
    random.seed(31)
    os.makedirs(SIM, exist_ok=True)
    with open(os.path.join(SIM, "cc_in.hex"), "w") as f:
        with open(os.path.join(SIM, "cc_expected.hex"), "w") as fe:
            for _ in range(ncase):
                ...
                r = dot_ref(a, b)
                # ожидаемый f32 результат = r.to_float() округлённый к float32
                exp = f32(r.to_float())
                fe.write(f"{f32_bits(exp):08x}\n")
    print(f"Сгенерировано {ncase} случаев")
...
def run_sim():
    files = [
        ...
        os.path.join(RTL, "rtl", "compute_core.sv"),
        os.path.join(RTL, "tb", "tb_compute_core.sv"),
    ]
    ...
    run(f'"{xelab}" tb_compute_core -debug typical')
    run(f'"{xsim}" tb_compute_core -runall')


def verify(tol=2):
    exp = [int(l.strip(), 16) for l in open(os.path.join(SIM, "cc_expected.hex")) if l.strip()]
    got = [int(l.strip(), 16) for l in open(os.path.join(SIM, "cc_out.hex")) if l.strip()]
    if len(exp) != len(got):
    ...
    if bad == 0:
        print(f"COMPUTE(N={N}): все {len(exp)} совпали (PASS, tol={tol})")
        return True
    print(f"COMPUTE: несовпадений {bad}/{len(exp)}")
    return False
