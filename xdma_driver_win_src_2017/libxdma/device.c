...
static BOOLEAN IsConfigBAR(IN PXDMA_DEVICE xdma, IN UINT idx) {
    ...
    UINT32 interruptID, configID;

    if (xdma->bar[idx] == NULL) {
        return FALSE;
    }

    // Критично: проверяем, что BAR достаточно велик для чтения регистров
    // IRQ_BLOCK_OFFSET = 0x2000, CONFIG_BLOCK_OFFSET = 0x3000
    // Если BAR меньше 0x3000 + sizeof(XDMA_CONFIG_REGS) — чтение убьёт систему
    if (xdma->barLength[idx] < CONFIG_BLOCK_OFFSET + sizeof(XDMA_CONFIG_REGS)) {
        TraceWarning(DBG_INIT, "IsConfigBAR: BAR %u too small (%u bytes), need at least %llu",
                     idx, xdma->barLength[idx],
                     (ULONGLONG)(CONFIG_BLOCK_OFFSET + sizeof(XDMA_CONFIG_REGS)));
        return FALSE;
    }
    ...
    interruptID = irqRegs->identifier & XDMA_ID_MASK;
    configID = cfgRegs->identifier & XDMA_ID_MASK;
...
static void GetRegisterModules(IN PXDMA_DEVICE xdma) {
    ...
    ULONG len = xdma->barLength[xdma->configBarIdx];

    // SGDMA_COMMON_BLOCK_OFFSET = 0x6000
    if (len >= SGDMA_COMMON_BLOCK_OFFSET + sizeof(XDMA_SGDMA_COMMON_REGS)) {
        xdma->sgdmaRegs = (XDMA_SGDMA_COMMON_REGS*)(configBarAddr + SGDMA_COMMON_BLOCK_OFFSET);
    } else {
        xdma->sgdmaRegs = NULL;
        TraceWarning(DBG_INIT, "GetRegisterModules: BAR too small for SGDMA common regs");
    }

    // CONFIG_BLOCK_OFFSET = 0x3000
    if (len >= CONFIG_BLOCK_OFFSET + sizeof(XDMA_CONFIG_REGS)) {
        xdma->configRegs = (XDMA_CONFIG_REGS*)(configBarAddr + CONFIG_BLOCK_OFFSET);
    } else {
        xdma->configRegs = NULL;
        TraceWarning(DBG_INIT, "GetRegisterModules: BAR too small for config regs");
    }

    // IRQ_BLOCK_OFFSET = 0x2000
    if (len >= IRQ_BLOCK_OFFSET + sizeof(XDMA_IRQ_REGS)) {
        xdma->interruptRegs = (XDMA_IRQ_REGS*)(configBarAddr + IRQ_BLOCK_OFFSET);
    } else {
        xdma->interruptRegs = NULL;
        TraceWarning(DBG_INIT, "GetRegisterModules: BAR too small for interrupt regs");
    }
}
...
NTSTATUS XDMA_DeviceOpen(WDFDEVICE wdfDevice,
    ...
    if (xdma->configRegs == NULL) {
        TraceError(DBG_INIT, "XDMA_DeviceOpen: configRegs not found (BAR too small)");
        return STATUS_DEVICE_CONFIGURATION_ERROR;
    }
