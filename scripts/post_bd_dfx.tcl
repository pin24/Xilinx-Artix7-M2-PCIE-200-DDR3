...
if {[catch {current_project}] != 0} {
    # скрипт вызван отдельно — ищем проект
...
set_property -dict [list \
    ...
    CONFIG.CLK_DOMAIN {xdma_0/axi_aclk} \
...
# Примечание: SmartConnect в Low-Area Mode использует единый aclk/aresetn,
# отдельные M03_ACLK/M03_ARESETN НЕ создаются. Подключаем только AXI-интерфейс.
...
# SmartConnect Low-Area Mode: единый aclk/aresetn (M0x_ACLK не существует)
...
# ============ 4b. ASSOCIATED_BUSIF для axi_aclk ============
set old_busif [get_property CONFIG.ASSOCIATED_BUSIF [get_bd_pins xdma_0/axi_aclk]]
foreach newif {M_AXI_TDOT S_AXI_TDOT_REGS S_AXI_ICAP_REGS S_AXI_XADC_REGS} {
    if {[string first $newif $old_busif] == -1} {
        set old_busif [string trim "$old_busif $newif"]
    }
}
set_property -dict [list CONFIG.ASSOCIATED_BUSIF $old_busif] [get_bd_pins xdma_0/axi_aclk]

# ============ 5. Экспорт такта PCIe-домена ============
...
proc _clk_connect {port_name pin_name} {
    ...
    if {[llength $npin] == 0} {
        ...
        # pin уже подключён к сети — добавляем порт к той же сети
