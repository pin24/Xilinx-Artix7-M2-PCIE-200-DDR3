# ============================================================================
# Full bitstream build: XDMA + DDR3 + троичное ядро dot (NUM_MAC=32)
# Synth -> Impl -> write_bitstream. No board programming.
# Run: vivado -mode batch -source scripts/build_full_bitstream.tcl
# ============================================================================
...
set logfile     C:/A7_M2/EXAMPLES/XDMA_DDR3/scripts/build_full.log

set NUM_MAC 32

open_project ${proj_dir}/${proj_name}/${proj_name}.xpr

# reset прошлых прогонов
reset_run synth_1 -quiet
reset_run impl_1 -quiet

# ---- top: новый wrapper с ядром ----
set_property top xdma_ddr3_core_top [current_fileset]

# ---- добавить RTL ядра и интеграции (без дубликатов) ----
foreach f [list \
    ...
    if {[get_files -all -quiet [file tail $f]] eq ""} {
        add_files -norecurse $f
        puts "=== added: $f ==="
    } else {
        puts "=== already in project: [file tail $f] ==="
    }
}

set_property generic NUM_MAC=$NUM_MAC [current_fileset]

# ---- constraints (правильный PROCESSING_ORDER) ----
...
# ---- полный flow: синтез -> имплементация -> bitstream ----
...
# ---- отчёты по успешному impl ----
open_run impl_1
report_timing_summary -file ${proj_dir}/scripts/timing_full.rpt
report_utilization       -file ${proj_dir}/scripts/util_full.rpt
report_utilization -hierarchical -file ${proj_dir}/scripts/util_full_hier.rpt
report_route_status
close_design

set bit [get_property DIRECTORY [get_runs impl_1]]
puts "=== BITSTREAM DIR: ${bit} ==="
puts "=== BUILD OK ==="
close_project
